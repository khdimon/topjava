...
public class UserMealsUtil {
  ...
  public static void main(String[] args) {
    ...
    List<UserMealWithExceed> meals = getFilteredWithExceeded(mealList,
...
//        .toLocalTime();
  ...
  public static List<UserMealWithExceed> getFilteredWithExceeded(List<UserMeal> mealList,
    ...
    Map<LocalDate, Integer> mealListByDays = mealList.stream()
        .collect(Collectors.groupingBy(UserMeal::getDate,
            Collectors.summingInt(UserMeal::getCalories)));
    return mealList.stream()
        .filter((m) -> TimeUtil.isBetween(m.getDateTime().toLocalTime(), startTime, endTime))
        .map((m) -> new UserMealWithExceed(m.getDateTime(), m.getDescription(), m.getCalories(),
            mealListByDays.get(m.getDateTime().toLocalDate()) > caloriesPerDay))
